...
class PANNsPresenceWrapper(nn.Module):
    ...
    def forward(self, wav):
        ...
        # Learned heads (if fine-tuned, else random; we blend with audioset)
        voice_head = torch.sigmoid(self.voice_head(embed)).squeeze(-1)  # (B,)
        music_head = torch.sigmoid(self.music_head(embed)).squeeze(-1)

        # Blend: if pretrained loaded, trust audioset 70% + head 30%; else 50/50 but head random -> will be 0.5
        if self.pretrained_loaded:
            voice_present = 0.7*voice_audioset + 0.3*voice_head
            music_present = 0.7*music_audioset + 0.3*music_head
        else:
