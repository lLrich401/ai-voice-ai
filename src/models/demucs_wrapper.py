...
class HTDemucsSeparator:
    def __init__(self, device="cpu", model_name="htdemucs", verbose=False):
        self.device = device
        ...
        if HAS_DEMUCS:
            ...
            if verbose: print("demucs not installed, using hpss")
...
def get_separator(device="cpu", verbose=False):
    global _global_separator
    if _global_separator is None:
        _global_separator = HTDemucsSeparator(device=device, verbose=verbose)
